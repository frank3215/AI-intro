...
epsilons = [.05, .1]
pretrained_model = "ModelM7/0.pth"
KERNEL_SIZE = 7
if(KERNEL_SIZE == 3):
...
def pgd_attack(model, orig_image, epsilon, target, step = .01):
    if epsilon == 0: return orig_image
    image = orig_image.detach() + 0.001 * torch.randn(orig_image.shape).detach()
    i = 0
    while i < epsilon:
        image = image.detach()
        ...
        image = fgsm_attack(image, step, data_grad)
        image = torch.min(torch.max(image, orig_image - epsilon), orig_image + epsilon)
...
def test( model, device, test_loader, epsilon ):
    ...
    for i, (data, target) in enumerate(test_loader):
        ...
        perturbed_data = pgd_attack(model, data.clone(), epsilon, target)
